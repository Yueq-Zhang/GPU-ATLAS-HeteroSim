#!/usr/bin/env python3
"""Validate the four P15c identity-untranslated coupled GPU Artifacts."""
...
QUALIFICATION_DIRS = {
    "attention_norm": "accel-sim-rtx3070-attention-norm-shared-hbdram-identity",
    "qkv_projection": "accel-sim-rtx3070-qkv-shared-hbdram-identity",
    "rope": "accel-sim-rtx3070-rope-shared-hbdram-identity",
    "causal_attention": (
        "accel-sim-rtx3070-causal-attention-shared-hbdram-identity"
    ),
}
...
def main() -> None:
    ...
    if set(catalog.required_operators) != set(OPERATORS):
        raise ValueError("P15c catalog required operator set is not frozen")
    if len(catalog.artifacts) != len(OPERATORS):
        raise ValueError("P15c catalog must contain exactly four GPU Artifacts")
    by_operator = {
        artifact.compatibility_key.operator: artifact
        for artifact in catalog.artifacts
    }
    if set(by_operator) != set(OPERATORS):
        raise ValueError("P15c catalog operator coverage is incomplete")
    ...
    for operator in OPERATORS:
        ...
        qualification = _mapping(
            artifact.payload["qualification"], "qualification"
        )
        if (
            ...
            or execution.get("global_pa_binding_ready") is not False
            or artifact.request_cycle_ready
            or qualification.get("performance_eligible") is not False
            or address.get("virtual_memory_mode") != "identity_untranslated"
        ):
            raise ValueError(f"invalid P15c readiness boundary: {operator}")

        record_path = root / QUALIFICATION_DIRS[operator] / "qualification_record.json"
        record = _load(record_path)
        trace_id = artifact.artifact_id.removesuffix(
            ".shared_hbdram_identity_v1"
        )
        if (
            ...
            raise ValueError(f"invalid coupled qualification: {record_path}")
        ...
        if (
            ...
            raise ValueError(f"request conservation failed: {operator}")
        records[operator] = {
    ...
    output = {
        "schema_version": "hetero-p15c-coupled-gpu-qualification/v1",
        "status": "passed",
        "scope": {
    ...
    print(f"P15c coupled GPU qualification passed: {destination}")
