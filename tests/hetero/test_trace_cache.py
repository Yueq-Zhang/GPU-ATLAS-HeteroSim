...
def test_trace_cache_keeps_accel_sim_v2_tracez_files(tmp_path) -> None:
    source = tmp_path / "source-v2"
    source.mkdir()
    (source / "kernel-1.tracez").write_bytes(b"tracez")
    (source / "kernelslist.g").write_text("kernel-1.tracez\n", encoding="utf-8")
    manifest_path = source / "manifest.json"
    manifest_path.write_text(
        json.dumps(
            {
                "schema_version": "hetero-trace-manifest/v1",
                "trace_id": "vector-add-v2",
                "trace_semantics": "functional",
                "replay_safe": False,
                "qualification_record": None,
                "kernels_list": "kernelslist.g",
                "capture": {"tool": "nvbit", "version": "1.8"},
                "compilation": {"target_sm": 86},
                "address_ranges": [],
            }
        ),
    ...
    cached = TraceCache(tmp_path / "cache").register(manifest_path)
    assert (cached.kernels_list.parent / "kernel-1.tracez").read_bytes() == b"tracez"
